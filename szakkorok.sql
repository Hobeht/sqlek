MariaDB [(none)]> create database szakkorok;
Query OK, 1 row affected (0.002 sec)

MariaDB [(none)]> create table diak
    -> (
    -> azon int unsigned primary key,
    -> nev varchar(100),
    -> evfolyam int unsigned,
    -> betujel varchar(1));
ERROR 1046 (3D000): No database selected
MariaDB [(none)]> use szakkorok;
Database changed
MariaDB [szakkorok]> create table diak
    -> (
    -> azon int unsigned primary key,
    -> nev varchar(100),
    -> evfolyam int unsigned,
...
MariaDB [szakkorok]> create table szakkor
...
MariaDB [szakkorok]> create table jelentkezes
    -> ( azon int unsigned not null primary key,
    -> diakazon int not null references diak(azon),
    -> szakazon int not null references szakkor(azon));;
...
MariaDB [szakkorok]> create table jelentkezes
    -> ( azon int unsigned not null primary key,
    -> diakazon int not null references diak(azon) on delete cascade,
    -> szakazon int not null references szakkor(azon) on delete cascade);
ERROR 1050 (42S01): Table 'jelentkezes' already exists
MariaDB [szakkorok]> insert into diak( azon, nev, evfolyam, betujel)
    ->
...
ERROR 1064 (42000): You have an error in your SQL syntax; check the manual that corresponds to your MariaDB server version for the right syntax to use near '1, 'Kis Lilla Laura', '11', 'D'),
(2, 'Hídvégi Antal', '11', 'B'),
(3, 'Ill...' at line 3
MariaDB [szakkorok]> Bye
Ctrl-C -- exit!
PS C:\Users\sarkozi_robert> ^C
...
MariaDB [(none)]> use szakkorok;
Database changed
MariaDB [szakkorok]> insert into diak( azon, nev, evfolyam, betujel) values
    -> (1, 'Kis Lilla Laura', '11', 'D'),
    -> (2, 'Hídvégi Antal', '11', 'B'),
    -> (3, 'Illés Anna Noémi', '11', 'F'),
    -> (4, 'Aszódi Anna', '10', 'B'),
    ...
    -> (306, 'Mihályi Bence', '8', 'B'),
    -> (307, 'Andocsi Brúnó', '9', 'A');
Query OK, 307 rows affected (0.016 sec)
Records: 307  Duplicates: 0  Warnings: 0

MariaDB [szakkorok]> insert into jelentkezes (azon, diakazon, szakazon) values
    ->
    -> (1, '1', '1'),
    -> (2, '2', '1'),
    -> (3, '3', '1'),
    ...
    -> (491, '89', '40'),
    -> (492, '307', '40');
Query OK, 492 rows affected (0.005 sec)
Records: 492  Duplicates: 0  Warnings: 0

MariaDB [szakkorok]>
MariaDB [szakkorok]> insert into szakkor( azon, mk, nev, tanar) values
    ->
...
Query OK, 40 rows affected (0.013 sec)
Records: 40  Duplicates: 0  Warnings: 0

MariaDB [szakkorok]> select nev from szakkorok order by nev;
ERROR 1146 (42S02): Table 'szakkorok.szakkorok' doesn't exist
MariaDB [szakkorok]> select nev from diak where evfolyam=10 and betujel='C' order by nev;
+------------------------+
| nev                    |
+------------------------+
| Bérdy Máté             |
| Derék Szilvia Boglárka |
| Koltai Dóra            |
| Lovász Laura           |
| Márta Bálint           |
| Mészáros Anna          |
...
MariaDB [szakkorok]> select nev, tanar from szakkor where nev like "%programoz%" and nev like "%robotika%";
Empty set (0.002 sec)

MariaDB [szakkorok]>select szakkor.nev,szakkor.tanar,szakkor.mk,Count(*)
from szakkor inner join jelentkezes on szakkor.azon=jelentkezes.szakazon
group by szakkor.nev
having Count(*)>9;

select evfolyam,  betujel
FROM ((jelentkezes
INNER JOIN diak ON jelentkezes.diakazon = diak.azon)
INNER JOIN szakkor ON jelentkezes.szakazon = szakkor.azon)
where szakkor.mk="Matematika"
group by diak.evfolyam,diak.betujel
;
select szakkor.nev
from ((jelentkezes inner join diak on jelentkezes.diakazon = diak.azon) inner join szakkor on jelentkezes.szakazon = szakkor.azon)
group by szakkor.nev, evfolyam
having count(*) = 2 and (evfolyam = 10 or evfolyam = 11);
